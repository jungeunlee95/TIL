...
for t in range(1, T+1):
    ...
    percent = []
    for _ in range(N):
        percent.append(list(map(int, input().split())))

    a = itertools.permutations([i for i in range(N)], N)
    max1 = 0
    for i in a:
        print(i)
        sum1 = 1
        for j in range(len(i)):
            a = percent[j][i[j]]*0.01
            sum1 *= a
        if sum1*100 > max1:
            max1 = sum1*100
    print("#{} {:.6f}".format(t, max1))
