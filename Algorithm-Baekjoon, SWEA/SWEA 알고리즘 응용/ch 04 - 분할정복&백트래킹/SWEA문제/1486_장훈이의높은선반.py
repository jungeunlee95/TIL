...
어느 날 장훈이는 자리를 비웠고, 이 서점에 있는 N명의 점원들이 장훈이가
선반 위에 올려놓은 물건을 사용해야 하는 일이 생겼다.
...
[출력]
각 테스트 케이스마다 첫 번째 줄에는 ‘#t’(t는 테스트 케이스 번호를 의미하며 1부터 시작한다)를 출력하고,
만들 수 있는 높이가 B 이상인 탑 중에서 탑의 높이와 B의 차이가 가장 작은 것을 출력한다.

[예제 풀이]
테스트 케이스의 경우 키가 3, 3, 5, 6인 점원이 탑을 만들면 높이가 17(3 + 3 + 5 + 6)이 된다.
높이가 16인 탑은 만들 수 없으므로 높이가 17인 탑이 문제에서 구하려는 탑의 높이이다.
따라서 17 – 16 = 1이 답이 된다.
'''
import sys
import itertools
...
#     N, B = map(int,input().split())
#     a = list(map(int,input().split()))
#     b = itertools.permutations([i for i in range(N)], N)
#     min = 99999
#     for i in b :
#         sum1 = 0
#         if min == B : break
#         for j in range(N):
#             if sum1 > B or sum1 > min:
#                 break
#             sum1 += a[i[j]]
#         if sum1 < min:
#             min = sum1
